...
class TestCoverageTranslation(unittest.TestCase):
    ...
    @patch("os.path.exists", return_value=True)
    @patch("os.remove")
    def test_handle_pivot_pass_success_with_en_target(self, mock_remove, mock_exists, mock_log, mock_save, mock_unreg, mock_reg, mock_popen, mock_file):
        mock_proc = MagicMock()
        ...
        missing = ["en", "fr"]
        res_data, res_code = translation._handle_pivot_pass([{"text": "salut", "start": 0, "end": 1}], "ro", "folder", "base", missing, "ron_Latn", [MagicMock()])
    ...
    @patch("os.path.exists", return_value=True)
    @patch("os.remove")
    def test_execute_translation_workers_orphaned(self, mock_remove, mock_exists, mock_log, mock_poll, mock_unreg, mock_reg, mock_popen, mock_file):
        mock_proc = MagicMock()
