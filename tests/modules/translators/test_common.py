...
def test_import_transformers_module_preserves_torchaudio_imports_after_error():
    ...
    try:
        ...
        mocked_import = MagicMock(side_effect=mock_import)
        with patch("modules.translators.common.importlib.import_module", mocked_import):
            res = common.import_transformers_module()
            assert res is not None
            mocked_import.assert_any_call("torchaudio")
            mocked_import.assert_any_call("transformers")
...
def test_import_transformers_module_when_torchaudio_already_in_modules():
    ...
    try:
        ...
        mocked_import = MagicMock(return_value=object())
        with patch("modules.translators.common.importlib.import_module", mocked_import):
            res = common.import_transformers_module()
            assert res is not None
            mocked_import.assert_any_call("torchaudio")
