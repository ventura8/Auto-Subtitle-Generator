...
class TestCoverageAutoSubtitle(unittest.TestCase):
    ...
    def test_load_nvidia_paths_torch_fail(self):
        with (
            ...
            patch(
                "builtins.__import__",
                side_effect=lambda name, *args, **kwargs: exec("raise ImportError()") if name == "torch" else MagicMock(),
            ),
        ):
    ...
    def test_check_resume_auto_scan_valid(self):
        def _exists(path):
            return path.endswith("base.fr.srt")

        with patch("os.path.exists", side_effect=_exists), patch("auto_subtitle.utils.parse_srt", return_value=[MagicMock()]):
    ...
    def test_collect_video_files_file_input_filters_unsupported_or_multilang(self):
        with patch("os.path.isfile", return_value=True), patch("os.path.basename", return_value="clip_multilang.mp4"):
            self.assertEqual(auto_subtitle.utils._collect_video_files("clip_multilang.mp4"), [])

        with patch("os.path.isfile", return_value=True), patch("os.path.basename", return_value="clip.txt"):
            self.assertEqual(auto_subtitle.utils._collect_video_files("clip.txt"), [])
    ...
    def test_format_total_processing_speed_known_duration(self):
        summary = auto_subtitle.utils._format_total_processing_speed(120.0, 60.0)
        self.assertIn("2.00x realtime", summary)

    def test_process_video_batch_logs_total_speed(self):
        with (
            patch("auto_subtitle.process_video"),
            patch("auto_subtitle.utils.get_audio_duration", return_value=120.0),
    ...
    def test_process_video_batch_logs_batch_summary_for_multiple_files(self):
        with (
            ...
            patch("auto_subtitle.utils.get_audio_duration", side_effect=[100.0, 200.0]),
            patch("auto_subtitle.time.time", side_effect=[0.0, 1.0, 11.0, 20.0, 50.0, 70.0]),
            patch("auto_subtitle.log"),
            patch("auto_subtitle.utils.log") as mock_utils_log,
        ):
            auto_subtitle.process_video_batch(["clip1.mp4", "clip2.mp4"], MagicMock(), None, None)

            self.assertTrue(any("[Batch Summary] Files: 2" in str(call.args[0]) for call in mock_utils_log.call_args_list))
            self.assertTrue(any("Succeeded: 1" in str(call.args[0]) for call in mock_utils_log.call_args_list))
            self.assertTrue(any("Failed: 1" in str(call.args[0]) for call in mock_utils_log.call_args_list))
            self.assertTrue(any("Media duration: 00:05:00" in str(call.args[0]) for call in mock_utils_log.call_args_list))
            self.assertTrue(any("[Batch Files]" in str(call.args[0]) for call in mock_utils_log.call_args_list))
            self.assertTrue(
                any("clip1.mp4" in str(call.args[0]) and "Status: succeeded" in str(call.args[0]) for call in mock_utils_log.call_args_list)
            )
            self.assertTrue(
                any("clip2.mp4" in str(call.args[0]) and "Status: failed" in str(call.args[0]) for call in mock_utils_log.call_args_list)
            )
