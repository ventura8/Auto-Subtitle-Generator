...
def _resolve_package_complexity(pkg, cls):
    ...
    if complexity == COMPLEXITY_UNAVAILABLE:
        pkg_name = pkg.get("name") or "<unknown>"
        raise ValueError(f"Complexity unavailable for package: {pkg_name}")
    return complexity
...
def _resolve_class_complexity(cls):
    ...
    if complexity == COMPLEXITY_UNAVAILABLE:
        class_name = cls.get("name") or "<unknown>"
        raise ValueError(f"Complexity unavailable for class: {class_name}")
    return complexity
