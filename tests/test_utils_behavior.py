...
class TestCoverageUtils(unittest.TestCase):
    def test_unregister_subprocess_removes_process(self):
        ...
        with patch("modules.utils.active_subprocesses", [proc]):
            ...
            self.assertEqual(utils.active_subprocesses, [])
    ...
    def test_handle_shutdown_windows_taskkill_branch(self):
        ...
        with (
            patch("modules.utils.active_subprocesses", [proc]),
    ...
    def test_init_console_exception(self):
        ...
        with patch("os.name", "nt"), patch("modules.utils.ctypes.windll", windll_stub, create=True):
    ...
    def test_setup_signal_handlers_windows(self):
        ...
        with (
            ...
            patch("modules.utils.ctypes.windll", windll_stub, create=True),
        ):
    ...
    def test_process_ffmpeg_line_exception(self):
        with patch("modules.utils.parse_timestamp", side_effect=ValueError("Parse fail")), patch("gc.collect") as mock_gc:
            utils._process_ffmpeg_line("time=00:00:01.00", 0, 100, "Desc")
            mock_gc.assert_called()

    def test_process_ffmpeg_line_no_total_duration(self):
        with patch("modules.utils.parse_timestamp", return_value=1.0), patch("modules.utils.print_progress_bar") as mock_bar:
            utils._process_ffmpeg_line("time=00:00:01.00", 0, 0, "Desc")
    ...
    def test_finalize_ffmpeg_progress_raises_called_process_error_on_nonzero_return(self):
        ...
        with patch("modules.utils.subprocess.CalledProcessError", FakeCalledProcessError):
            with self.assertRaises(FakeCalledProcessError):
                utils._finalize_ffmpeg_progress(process, ["ffmpeg"], 0.0, 0.0, "desc")

    def test_finalize_ffmpeg_progress_raises_runtime_error_when_called_process_error_invalid(self):
        process = MagicMock(returncode=1)
        process.returncode = 1
        with patch("modules.utils.subprocess.CalledProcessError", "invalid"):
            with self.assertRaises(RuntimeError):
                utils._finalize_ffmpeg_progress(process, ["ffmpeg"], 0.0, 0.0, "desc")

    def test_run_ffmpeg_progress_exception(self):
        with patch("subprocess.Popen", side_effect=OSError("Popen fail")):
    ...
    def test_extract_clean_audio_reuse(self):
        with (
            ...
            patch("modules.utils.run_ffmpeg_progress") as mock_run_ffmpeg_progress,
    ...
    def test_extract_clean_audio_fail(self):
        with (
            ...
            patch("modules.utils.get_audio_duration", return_value=123.45),
            patch("modules.utils.run_ffmpeg_progress", side_effect=RuntimeError("Extraction failed")),
            patch("modules.utils.log"),
        ):
            with self.assertRaises(RuntimeError):
    ...
    def test_extract_clean_audio_invalid_output_raises(self):
        with (
            ...
            patch("modules.utils.get_audio_duration", return_value=10.0),
    ...
    def test_get_cpu_name_exception(self):
        ...
        with patch("sys.platform", "win32"), patch.dict("sys.modules", {"winreg": winreg_stub}), patch("modules.utils.winreg", winreg_stub):
    ...
    def test_parse_srt_corrupted(self):
        with patch("modules.utils.validate_srt", return_value=False), patch("logging.getLogger"):
            self.assertEqual(utils.parse_srt("bad.srt"), [])

    def test_parse_srt_garbage_chunks(self):
        content = "NotADigit\n00:00:00,000 --> 00:00:01,000\nText\n\n2\nInvalidTime\nText"
        with patch("modules.utils.validate_srt", return_value=True), patch("builtins.open", mock_open(read_data=content)):
