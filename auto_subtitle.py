...
def _is_usable_language(language):
    ...
    return bool(normalized) and normalized not in {"und", "undetermined"}
...
def _build_embed_command(video_path, srt_files, normalized_ext, output_path, src_lang=None):
    ...
    cmd.extend(_build_primary_media_metadata_args(src_lang if src_lang else srt_files[0][1]))
    cmd.extend(_build_embed_metadata_args(srt_files))
    cmd.extend(["-loglevel", "info", output_path])
    return cmd


def _build_primary_media_metadata_args(src_lang):
    """Tag copied primary media streams with the detected source language."""
    source_language = _to_mux_language_code(src_lang)
    return [
        "-metadata:s:v:0",
...
def _to_mux_language_code(lang):
    """Convert ISO language key into standard ISO 639-2 3-letter mux code."""
    return config.to_mux_language_code(lang)
