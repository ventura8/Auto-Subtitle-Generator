...
def print_progress_bar(iteration, total, prefix='', suffix='', decimals=1, length=30, fill='█', empty='░', elapsed=None, speed=None, eta=None):
    """
...
class SystemOptimizer:
    ...
    def set_profile(self, profile_name, verbose=True):
        ...
        profiles = {
            "ULTRA": {"whisper_beam": 5, "whisper_compute": "float16", "nllb_batch": 32, "ffmpeg_threads": self.cpu_cores},
            "HIGH": {"whisper_beam": 5, "nllb_batch": 64, "ffmpeg_threads": self.cpu_cores},
            "MID": {"whisper_beam": 5, "nllb_batch": 32},
            "LOW": {"whisper_beam": 2, "nllb_batch": 8, "whisper_compute": "int8_float16"},
            "CPU_ONLY": {"whisper_beam": 2, "nllb_batch": 4, "whisper_compute": "int8", "device": "cpu"},
        }
...
def load_config():
    ...
    try:
        ...
        if "whisper" in config:
            ...
            if w_conf.get("use_prompt", True):
                ...
                if custom:
                    ...
                    log(f"[Config] Using Custom Prompt from config.")
                else:
                    log(f"[Config] Using Default Prompt (Enabled in config).")
            else:
                INITIAL_PROMPT = None
                log(f"[Config] Prompt Disabled in config.")
        ...
        if "hallucinations" in config:
            ...
            log(f"[Config] Loaded Hallucination Filters (Silence: {HALLUCINATION_SILENCE_THRESHOLD}, Rep: {HALLUCINATION_REPETITION_THRESHOLD})")
...
class NLLBTranslator:
    ...
    def load(self):
        ...
        log(f"  [Model-Load] Loading NLLB-200-3.3B on {self.device.upper()}...")
        try:
...
def cleanup_temp_files(folder, base_name, file_name):
    """Removes temporary audio files created during processing."""
    temp_wav = os.path.join(folder, f"{base_name}_temp.wav")
    pattern = f"{base_name}_temp"
    
    if os.path.exists(folder):
        for f in os.listdir(folder):
            if pattern in f and (".wav" in f or ".mp3" in f):
                if f == file_name:
                    continue
                try:
    ...
    if not prior_srt:
        ...
        try:
            ...
            if info.language_probability < 0.5:
                log(f"  [Warning] Low language confidence ({info.language_probability:.2f}). May indicate music/noise.", "WARNING")
            ...
            for seg in segments:
                ...
                if len(last_texts) >= HALLUCINATION_REPETITION_THRESHOLD:
                    if len(set(last_texts)) == 1:
                        if not hallucination_detected:
                            log(f"  [Warning] Hallucination detected: '{seg_text[:50]}...' repeating. Stopping.", "WARNING")
                ...
                text_preview = seg_text[:40] + "..." if len(seg_text) > 40 else seg_text
                print_progress_bar(
                    seg.end, total_dur, 
                    prefix="    -> ", 
                    suffix=f"Transcribing: {text_preview:<43}", 
                    length=30
                )
            ...
            # Ensure bar hits 100% visually
            print_progress_bar(100, 100, prefix="    -> ", suffix="Transcription Complete.                          ")
            print("")
    ...
    if not original_segments:
        ...
        try:
            ...
            if tasks_to_run:
                ...
                source_texts = [s.text.strip() for s in original_segments]
                batch_size = OPTIMIZER.config["nllb_batch"]
                
                log(f"  [AI] Translating to {len(tasks_to_run)} languages (Skipped {len(all_tasks) - len(tasks_to_run)})...")

                for i, (lang_key, info) in enumerate(tasks_to_run):
                    tgt_code = info["code"]
                    label = info["label"]
                    ...
                    while True:
                        try:
                            translated_lines = []
                            total_batches = (len(source_texts) + current_batch - 1) // current_batch
                            
                            for batch_idx, b_start in enumerate(range(0, len(source_texts), current_batch)):
                                batch = source_texts[b_start : b_start + current_batch]
                                results = translator.translate(batch, src_nllb, tgt_code)
                                translated_lines.extend(results)
                                
                                print_progress_bar(
                                    batch_idx + 1, total_batches, 
                                    prefix=f"    -> ", 
                                    suffix=f"Translating to {label} ({lang_key})...", 
                                    length=30,
                                    decimals=0
                                )
                            ...
                            tgt_srt_path = os.path.join(folder, f"{base_name}.{lang_key}.srt")
                            save_translated_srt(original_segments, translated_lines, tgt_srt_path)
                            generated_srts.append((tgt_srt_path, lang_key, label))
                            break

                        except (RuntimeError, MemoryError) as e:
                            is_cuda_oom = isinstance(e, RuntimeError) and "out of memory" in str(e).lower()
                            is_cpu_oom = isinstance(e, MemoryError)
                            
                            if is_cuda_oom or is_cpu_oom:
                                oom_type = "CUDA" if is_cuda_oom else "CPU"
                                print(f"\n      [!] {oom_type} OOM with batch {current_batch}. "
                                      f"Clearing memory and retrying with batch {current_batch // 2}...")
                                ...
                                current_batch //= 2
                                if current_batch < 1:
                                    log(f"  [Error] OOM even with batch 1 for {label}. Skipping.", "ERROR")
                                    break
                            else:
                                raise e
                    
                # DO NOT DELETE TRANSLATOR HERE - Managed by ModelManager
                # del translator
                # torch.cuda.empty_cache()
            else:
...
def main():
    ...
    custom_prompt_text = INITIAL_PROMPT
    ...
    if os.path.exists(target):
        ...
        if os.path.isdir(target):
            files = [os.path.join(target, f) for f in os.listdir(target) if os.path.splitext(f)[1].lower() in VIDEO_EXTENSIONS]
            for f in files:
