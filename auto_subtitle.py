...
def _get_resume_candidates(folder, base_name, forced_lang):
    ...
    source_candidates = _filter_resume_source_candidates(recorded_source_lang, discovered_languages)
    return prioritize_recorded_language(recorded_source_lang, source_candidates)


def _filter_resume_source_candidates(recorded_source_lang, discovered_languages):
    """Exclude configured translations when a recorded source language identifies them."""
    if not is_usable_language(recorded_source_lang):
        return discovered_languages
    return [language for language in discovered_languages if _is_resume_source_candidate(language, recorded_source_lang)]


def _is_resume_source_candidate(language, recorded_source_lang):
    """Return whether a discovered subtitle can represent the recorded source."""
    return language not in config.TARGET_LANGUAGES or language == recorded_source_lang
